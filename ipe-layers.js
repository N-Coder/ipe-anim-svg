 ...
 *   <div class="slides" data-ipe-no-auto>   — disable auto-fill globally
 *   <section data-ipe-page="N">             — select page by 1-based index
 *   <section data-ipe-page="Title">         — select page by title
 *   <section data-ipe-no-auto>              — disable auto-fill for this slide
...
const IpeLayers = (() => {
  ...
  return {
    ...
    async init(deck) {
      ...
      const globalNoAuto = slidesEl.hasAttribute('data-ipe-no-auto');
      ...
      await Promise.all(
        [...sectionPageMap.entries()].map(async ([section, pageInfo]) => {
          const noAuto = globalNoAuto || section.hasAttribute('data-ipe-no-auto');
